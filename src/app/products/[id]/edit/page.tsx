...
export default function EditProductPage({ params }: { params: { id: string } }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(`/api/products/${params.id}`, {
        method: "PUT", // QUAN TRỌNG: Dùng PUT để cập nhật
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(formData),
      });

      if (res.ok) {
        // Cập nhật xong thì quay về trang chi tiết sản phẩm đó
  ...
  if (fetching) return <div className="min-h-screen flex items-center justify-center">Loading product data...</div>;
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="mb-8 flex items-center justify-between">
          ...
          <div className="flex gap-3">
             <Link href={`/products/${params.id}`} className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50">
               Cancel
             </Link>
             <button 
